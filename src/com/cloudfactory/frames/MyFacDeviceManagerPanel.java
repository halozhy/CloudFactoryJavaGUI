...
public class MyFacDeviceManagerPanel extends JPanel {

	
	class MyButtonRender5 implements TableCellRenderer {
		...
		private void initButton() {
			...
			// 设置按钮的大小及位置。
			this.button.setBounds(0, 0, 70, 30);
			this.button2.setBounds(70, 0, 70, 30);
			this.button3.setBounds(140, 0, 70, 30);
...
//			this.button.setIcon(IconHandler.resizeIcon("./img/edit.png"));
			...
			this.button2.setIcon(IconHandler.resizeIcon("./img/edit.png"));
//			this.button2.setText("修改");
			this.button3.setIcon(IconHandler.resizeIcon("./img/delete.png"));
//			this.button3.setText("删除");
	 ...
	 * 
	 */
	class MyButtonEditor5 extends DefaultCellEditor {
		...
		public MyButtonEditor5(JTable table) {
		...
		private void initButton(JTable table) {
			...
			this.button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					MyButtonEditor5.this.fireEditingCanceled();
//					System.out.println("open/close " + table.getSelectedRow() + " " + table.getSelectedColumn());
					...
					dController.opencloseDevice(id);
...
//					System.out.println("modify " + table.getSelectedRow() + " " + table.getSelectedColumn());
					...
					DeviceController dController = DeviceController.getInstance("Device");
////					打开编辑窗口
					Device oDevice = dController.findDeviceCenter(id);
					if (oDevice.getSource() == 2) {
						JOptionPane.showMessageDialog(null, "该设备正在被租用，无法修改", "错误", JOptionPane.ERROR_MESSAGE);
					} else {
						MyModifyDeviceFrame.mainForModify(oDevice,userID);
					}
...
//					System.out.println("delete " + table.getSelectedRow() + " " + table.getSelectedColumn());
					...
					Device oDevice = dController.findDeviceCenter(id);
					if (oDevice.getSource() == 2) {
//						System.out.println("解租");
						int re=JOptionPane.showConfirmDialog(null, "该设备正在被租用，确认解租？", "提示", JOptionPane.OK_CANCEL_OPTION);
						if(re==0) {
							dController.releaseRentDevice(id, userID);
							MyFacDeviceManagerPanel.getInstance(userID).updateTable();
						}
					} else {
						int re = JOptionPane.showConfirmDialog(null, "确认删除？", "提示", JOptionPane.OK_CANCEL_OPTION);
						if (re == 0) {
							dController.removeMyDevice(id,userID);
							MyFacDeviceManagerPanel.getInstance(userID).updateTable();
						}
					}
...
//			this.button2.setText("删除");
	...
	private static MyFacDeviceManagerPanel instance;
	 ...
	 */
	public static MyFacDeviceManagerPanel getInstance(String id) {
		if (instance == null) {
			instance = new MyFacDeviceManagerPanel(id);
		}
		return instance;
	}
	 ...
	 * Create the panel.
	 */
	private MyFacDeviceManagerPanel(String id) {
		userID=id;
		...
		JButton btnRent = new JButton("租用");
		btnRent.setFont(new Font("微软雅黑", Font.PLAIN, 16));
				...
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
						.addGroup(groupLayout.createSequentialGroup()
							...
							.addPreferredGap(ComponentPlacement.UNRELATED)
							.addComponent(btnRent, GroupLayout.PREFERRED_SIZE, 115, GroupLayout.PREFERRED_SIZE))
			...
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(btnRent, GroupLayout.PREFERRED_SIZE, 31, GroupLayout.PREFERRED_SIZE))
					.addPreferredGap(ComponentPlacement.RELATED)
		...
		btnRent.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				DeviceRentFrame.run(userID);
			}
		});
	}

	public void iniTable() {
		String[] columnNames = { "序号", "设备编号", "设备名称", "设备类型", "设备规格", "设备描述", "设备状态", "租用状态", "所属工厂", "操作" };
//		String[][] cm2= {{"1","11","111"},{"2","22","222"},{"3","33","333"}};
		DeviceController dController = DeviceController.getInstance("Device");
		String[][] data = dController.getMyAll(userID);
//		for (String[] s : data) {
...
//		}
		...
		table.getColumnModel().getColumn(9).setCellEditor(new MyButtonEditor5(table));
		table.getColumnModel().getColumn(9).setCellRenderer(new MyButtonRender5());
